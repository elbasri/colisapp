...
class PasswordResetController extends Controller {
    ...
    public function sendResetLink(Request $request) {
        ...
        $message = 'Use This Link to Reset Password: ' . url('/') . '/customer/password/reset/' . $token;
    ...
    public function resetLink($token) {
        ...
        if (empty($reset) || $reset->status == 1) {
            ...
            return view('customer.passwords.reset', compact('reset'));
        }
    }
    ...
    public function passwordReset(Request $request) {
        ...
        if (empty($reset) || empty($user) || $reset->status == 1) {
            ...
            return redirect()->route('customer')->withErrors('Invalid Reset Link');
        } else {
            ...
            return redirect()->route('customer')->withSuccess('success', 'Password Changed');
        }
    }
