...
<div class="content p-4">
    <h2 class="mb-4" style="text-transform: uppercase;">{{__('All Branch Customer')}}
        <a href="{{ route('branchcustomer.create') }}" class="btn btn-primary btn-md float-right">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-12  mb-2">
                    <form method="GET" action="{{ route('branchcustomer.index') }}" class="form-inline float-right">
                        @csrf
            ...
            <table id="table" class="table table-hover table-bordered" cellspacing="0" width="100%">
                ...
                <tbody>
                    @forelse($branchCustomerList as $key=>$branchCustomer)
                    <tr>
                        <td>{{ $key+1 }}</td>
                        <td>{{ $branchCustomer->name }}</td>                       
                        <td>{{ $branchCustomer->email }}</td>
                        <td>{{ $branchCustomer->phone }}</td>
                        <td>{{ $branchCustomer->branch->name }}</td>
                        <td>
                            @if($branchCustomer->status  == 'Active')
                            ...
                            <div class="modal fade" id="changePassword{{ $branchCustomer->id }}" role="dialog" aria-labelledby="#changePassword" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <form method="POST" action="{{ route('branchcustomer.changepassword') }}">
                                            @csrf
            ...
            {{ $branchCustomerList->appends(['search'=>request()->search])->links() }}
        </div>
    </div>
</div>
