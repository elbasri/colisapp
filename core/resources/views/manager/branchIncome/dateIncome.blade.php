...
<div class="content p-4">
    <h2 class="mb-4" style="text-transform: uppercase;">{{ Auth::user()->branch->name }} {{__('Branch Income List')}}
        <a href="{{ route('manager.branch.income') }}" class="btn btn-primary btn-md float-right">
            <i class="fa fa-backward"></i>   {{__('Go Back')}}
        </a>
    </h2>
    <div class="card mb-4">
        <div class="card-body">
                ...
                <tbody class="branch_income">
                    ...
                    <tr>
                        ...
                        <td>{{ $branchIncome->total_balance }}&nbsp;{{ $gs->base_currency }}</td>
                         <td><a href="{{ route('manager.branch.income.customer',$branchIncome->payment_receiver_id) }}">{{ $branchIncome->payment_receiver->name }} [ {{ $branchIncome->payment_receiver->type }} ]</a></td>
                    </tr>
                    @empty
                    <tr>
