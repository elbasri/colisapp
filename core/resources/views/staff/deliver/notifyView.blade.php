        ...
        <div class="card-body">
            <form action="#" id="notifyForm">
                <div class="row">                
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            <button type="button" id="searchCurrierBtn" onclick="javascript:searhCurrier()" class="btn btn-primary btn-block btn-lg search-btn"><i class='fa fa-search'></i></button>
                        </div>
                    </div>
                </div>
            </form>
            ...
            <div class="modal fade" id="receive" role="dialog" aria-labelledby="#" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <form method="POST" action="{{ route('send.notification.currier') }}">
                            @csrf
    ...
    function searhCurrier() {
        ...
        if (code !== '') {
            ...
            $(".invoice_unique").each(function () {
                if ($(this).text() == code) {
                    flag++;
                    toastr.error("Already selected this currier");
                }
            });
            if (flag == 0) {
                $.ajaxSetup({
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{url("staff/currier/deliver/notification")}}',
                    dataType: 'json',
                    data: {
                        code: code
                    },
                    success: function (response) {
                        var obj = response;
                        console.log(obj.currier);
                        if (obj.output === "success") {
                            $.each(obj.currier, function (key, Event) {
                                var fieldHTML = '';
