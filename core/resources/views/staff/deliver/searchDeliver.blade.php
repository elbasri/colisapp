...
<div class="content p-4">
    <h2 class="mb-4" style="text-transform: uppercase;">{{__('Search Courier')}}
    </h2>
    <div class="card mb-4">
        <div class="card-body">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                                    ...
                                    <div class="modal-content">
                                        <form method="POST" action="{{ route('currier.receive') }}">
                                            ...
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="receive{{ $currier->id }}"><i class="fa fa-download"></i>{{__('Deliver Courier')}}</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="modal-body">
                                                <p><strong>{{__('Are you sure! You deliver this Courier ?')}}</strong></p>
                            ...
                            <button type="button" class="btn btn-primary btn-sm delete_button" data-toggle="modal" data-target="#receive{{ $currier->id }}">
                                <i class="fa fa-cart-arrow-down"></i>{{__('Deliver Courier')}}  
                            </button>
                            <div class="modal fade" id="receive{{ $currier->id }}" role="dialog" aria-labelledby="#" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <form method="POST" action="{{ route('currier.receive') }}">
                                            ...
                                            @method('PUT')
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="receive{{ $currier->id }}"><i class="fa fa-money-bill-alt"></i>{{__('Make Payment')}}</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                            ...
                            <span class="btn btn-sm btn-success text-uppercase">{{ $currier->status }}</span>
                            @endif
                            <a href="{{ route('currier.invoice',$currier->invoice_id) }}"><button class="btn btn-primary btn-sm"> <i class="fa fa-eye"></i>{{__('Courier Invoice')}} </button></a>                           
                        </td> 
                    </tr>
                    @empty
                    <tr>
