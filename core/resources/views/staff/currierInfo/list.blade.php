...
<div class="content p-4">
    <h2 class="mb-4" style="text-transform: uppercase;">{{__('All Currier List')}}
        <a href="{{ route('currier.create') }}" class="btn btn-primary btn-md float-right">
            <i class="fa fa-list"></i>{{__('Add Courier')}}   
        </a>
    </h2>
    <div class="card mb-4">
        <div class="card-body">
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-danger btn-sm delete_button" data-toggle="modal" data-target="#receive{{ $currier->id }}">
                                <i class="fa fa-check"></i>{{__('Receive Courier')}}  
                            </button>
                                    ...
                                    <div class="modal-content">
                                        <form method="POST" action="{{ route('currier.receive') }}">
                                            ...
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="receive{{ $currier->id }}"><i class="fa fa-download"></i>&nbsp;{{__('Receive Courier')}}</h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <div class="modal-body">
                                                <p><strong>{{__('Are you sure! You receive this Courier')}} ?</strong></p>
                        ...
                        <td class="text-center">
                            ...
                            @else
                            <p>{{__('Received at')}} <br>[ {{ $currier->created_at->toDateString() }} ]</p>
                            @endif
                        </td>
                        @endif
                        <td>
