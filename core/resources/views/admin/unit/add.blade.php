...
<div class="content p-4">
    <h2 class="mb-4" style="text-transform: uppercase;">{{__('Create New Unit')}}
        <a href="{{ route('unit.index') }}" class="btn btn-primary btn-md float-right">
            <i class="fa fa-list"></i>{{__('View Unit')}}   
        </a>
    </h2>
    <div class="card mb-4">
        <div class="card-body">
            <form action="{{ route('unit.store') }}" method="POST">
                ...
                <br>
                <div class="col-md-12">
                    <button type="submit" class="btn btn-primary btn-block btn-lg">{{__('Create New Unit')}}</button>
                </div>
            </form>
        </div>
    </div>
