...
<div class="content p-4">
    <h2 class="mb-4" style="text-transform: uppercase;">{{__('Edit Branch Manager')}}
            ...
            <form action="{{ route('branchmanager.update',$branchmanager->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-12  container-fluid">
                    <div class="form-group">
                        ...
                        <input class="form-control form-control-lg mb-3" type="text" name="name"  value="{{ $branchmanager->name ?? old('name') }}" placeholder="{{__('Branch Manager Name')}}">
                    </div>
                </div>
                <div class="col-md-12 container-fluid">
                    <div class="form-group">
                        <label for="email" style="text-transform: uppercase;"><strong>{{__('Email')}}&nbsp;<span class="mark">*</span></strong></label>
                        <input class="form-control form-control-lg mb-3" type="email" name="email"  value="{{ $branchmanager->email ?? old('email') }}" placeholder="{{__('Branch Manager Email')}}">
                    </div>
                </div>
                <div class="col-md-12 container-fluid">
                    <div class="form-group">
                        <label for="phone" style="text-transform: uppercase;"><strong>{{__('Phone')}}</strong></label>
                        <input class="form-control form-control-lg mb-3" type="text" name="phone"  value="{{ $branchmanager->phone ?? old('phone') }}" placeholder="{{__('Branch Manager Phone')}}">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                ...
                <br>
                <div class="col-md-12">
                    <button type="submit" class="btn btn-primary btn-block btn-lg">{{__('Update Manager')}}</button>
                </div>
            </form>
        </div>
    </div>
