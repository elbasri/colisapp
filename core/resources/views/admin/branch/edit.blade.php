...
<div class="content p-4">
    <h2 class="mb-4" style="text-transform: uppercase;">{{__('Edit Branch')}}
            ...
            <form action="{{ route('branch.update',$branch->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-12 container-fluid">
                    <div class="form-group">
                        ...
                        <input class="form-control form-control-lg mb-3" type="email" name="email"  value="{{ $branch->email ?? old('email') }}" placeholder="{{__('Branch Email')}}">
                    </div>
                </div>
                <div class="col-md-12 container-fluid">
                    <div class="form-group">
                        <label for="phone" style="text-transform: uppercase;"><strong>{{__('Phone')}}</strong></label>
                        <input class="form-control form-control-lg mb-3" type="text" name="phone"  value="{{ $branch->phone ?? old('phone') }}" placeholder="{{__('Branch Phone')}}">
                    </div>
                </div>
                <div class="col-md-12 container-fluid">
                    <div class="form-group">
                        <label for="address" style="text-transform: uppercase;"><strong>{{__('Address')}}</strong></label>
                        <textarea class="form-control" rows="3" name="address" placeholder="{{__('Branch Address')}}">{{ $branch->address ?? old('address') }}</textarea>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                ...
                <br>
                <div class="col-md-12">
                    <button type="submit" class="btn btn-primary btn-block btn-lg">{{__('Update Branch')}}</button>
                </div>
            </form>
        </div>
    </div>
